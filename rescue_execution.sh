...
#ねねっちビューアーログの削除
nenedir=(`find $git_address -name RioneViewer2`)
...
if [ ! -z $nenedir ]; then
	...
	do
		...
		rm -Rf $nene/Data/Area &>/dev/null
		rm -Rf $nene/Data/Human &>/dev/null
		rm -Rf $nene/Data/Score &>/dev/null
		rm -Rf $nene/Data/Time &>/dev/null
		
		mkdir $nene/Data/Area &>/dev/null
		mkdir $nene/Data/Human &>/dev/null
		mkdir $nene/Data/Score &>/dev/null
		mkdir $nene/Data/Time &>/dev/null
